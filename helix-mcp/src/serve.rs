...
- `helix_get_signature_help(line, column, path?)` — LSP function-call signature with active parameter index. Use when generating code that calls a function and you need argument order in context. allow_insert_mode defaults to true — signature help is designed for mid-typing.
...
- `helix_get_jumplist()` — read the active view's jumplist (ordered history of cursor positions the user has visited). Each entry has path/line/column and `is_current` flagging where the user is now. Use to retrace recent navigation or as a precursor to `helix_jump`.
- `helix_jump(offset)` — step along the jumplist by N entries. Negative is backward (`-1` = "go back one"), positive is forward. Behaves like vim's Ctrl-O / Ctrl-I. View recenters on the destination.
- `helix_get_code_actions(line, column, end_line?, end_column?, path?, only?)` — list LSP code actions (quick-fixes, refactors, organize-imports, …) at a position or range. Returns opaque `id`s for each. When fixing a diagnostic, prefer this over hand-editing — the LSP's transform is always more accurate. Pass `only: ["quickfix"]` to filter to fix-only.
- `helix_apply_code_action(action_id)` — apply a previously-fetched code action by its opaque id. Errors with code -32006 (CodeActionStale) if the buffer changed since the id was issued — call helix_get_code_actions again and retry. Errors with code -32007 (CodeActionUnknown) for evicted or never-issued ids.
